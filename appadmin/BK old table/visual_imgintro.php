...
function elimina_imma(imm) {
	...
	if(confirm("Vuoi veramente eliminare l'Immagine?"))
	{
  		...
  		location.href = 'visual_imgintro.php?ID_imma='+imm+'&azione=elimina_imma';
    ...
    <?php include "include/top_bar.php"; ?>
 	...
 	<table border="0" cellspacing="0" cellpadding="0">
		...
		if($azione == "elimina_imma" ){
			...
			//elimino l'immagine
			$strEliminaImm = "DELETE FROM immintro
							WHERE ID_imm='{$ID_imma}'";			
						
			$resEliminaImm = mysql_query($strEliminaImm) or die(mysql_error());
						
						
			if( (!@unlink($dir.basename($res_URL['URL']))) && (!@unlink($dir."intro_".basename($res_URL['URL'])))  ) {
				$colEsito = "#ff0000";
				$strEsito = "ERRORE! Immagine non eliminata dalla directory!".$dir."intro_".basename($res_URL['URL']);
			} else  {
	...
	if ( ($uploadfile_name == "") && ($checks == 1)) {
		...
		if ($uploadfile_size < $dim_file ) {
			...
			if(move_uploaded_file($_FILES['uploadfile']['tmp_name'], $dir.$random.$_FILES['uploadfile']['name'])){
				...
				if ($azione == "modifica_do") {
					...
					if ($azione == "modifica_do") {
						...
						$str_insert_attach = "INSERT INTO immintro (ID_imm, Label, URL, Attiva, Ordine)
			...
			if(($extension == "image/gif") || ($extension == "image/pjpeg") || ( $extension == "image/jpeg")) {	
					...
					//Creo l'immagine intro
					$immagine->fill($max_l,$max_h);
					$immagine->save($dir."intro_".$random.$_FILES['uploadfile']['name']);
					//chmod($dir."thumb_".$_FILES['uploadfile']['name'], 0777);
		...
		<tr>
			...
			if (strcmp($azione,"modifica") == 0 ) {
				...
				$str_lab = "Modifica Immagine INTRO (1000x613px) ";
			} else {
				$str_lab = "Aggiungi Immagine INTRO (1000x613px) ";
			}	
              ...
              <tr>
                ...
                <td colspan="3" class="bodyTxt"><table width="100%" class="contentTable" cellspacing="0" cellpadding="0">
                    <tr>
                      <td align="left" valign="top">
					  	<?php if (strcmp($azione,"modifica") == 0 ) { ?>
                      		<img src="<?php echo "/admin/public/intro_".basename($res_immDett['URL']); ?>" border="0" width="150px" >
						<?php } ?>
    	...
    	<tr>
  		...
  		<tr>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabellaMenuBordoBlu">
			 ...
			 $str_conta = "SELECT count(*) FROM immintro ";
			...
			if ( $tot_news > 0 ) {
				...
				while ( $res_cat = mysql_fetch_array($query_cat)){			
					...
					$url_img = "/admin/public/intro_".basename($res_cat['URL']);
				...
				<tr> 
					...
					<td width="82" align="center" valign="middle" class="sezioneMenu" bgcolor="<?php echo $colore_celle;?>" >
						<a href="visual_imgintro.php?azione=modifica&ID_imma=<?php echo $res_cat['ID_imm']; ?>" class="sezioneMenu">
                 ...
                 <tr bgcolor="<?php echo COLORE_HEADER_TABLE; ?>"> 
                    ...
                    for ($j = 0; $j < $n_pagine; $j++){
                        ...
                        ?>
                            -
                            <a href="visual_imgintro.php" class="sezioneMenu">
                        <?php
                            echo $pagine[$j];
                            echo "</a>";	
                        } else {
